...
# R9 Central v3.3 - интерфейс управления C_2 (HF Space node)
#
# Архитектура: R23 (Формула -> Принцип -> Аксиома -> Слова).
#
# v3.3 vs v3.2 (доработки):
# [fix]  Все умные кавычки заменены на прямые (U+201C/U+201D -> U+0022)
# [fix]  html.escape() для всех пользовательских строк в HTML
# [fix]  build_curl: корректные кавычки для Content-Type
# [new]  Валидация endpoint (только относительные пути)
# [new]  Rate limiting (1 запрос в 500 мс)
# [new]  Макс. длина ввода (4096 символов)
# [new]  Проверка Content-Type заголовка ответа
#
# Запуск: streamlit run r9_central.py
# Переменные окружения: C2_URL (default: https://rollannf-r9-c-2.hf.space),
#                       HF_TOKEN (опционально - идет в Authorization: Bearer)
...
def validate_endpoint(endpoint):
    # Разрешаем только относительные пути: /path/to/endpoint
    if not endpoint:
        return False, "Endpoint не может быть пустым"
    if not endpoint.startswith("/"):
        return False, "Endpoint должен начинаться с /"
    # Только буквы, цифры, подчеркивание, дефис, слэш, точка
    if not re.match(r"^/[a-zA-Z0-9_/ .-]*$", endpoint):
        return False, "Endpoint содержит недопустимые символы"
    # Запрещаем path traversal
    if ".." in endpoint:
        return False, "Path traversal запрещен"
    return True, None
...
def check_rate_limit():
    ...
    if elapsed < RATE_LIMIT_MS:
        return False, int(RATE_LIMIT_MS - elapsed)
...
<div style="text-align:center; margin: 1.2rem 0 1.4rem 0; position: relative; z-index: 1;">
    ...
    <div style="color: rgba(0,0,0,0.5); font-size: 13px; margin-top: 6px;">
        Канал связи с узлом C_2 · защищенный
    </div>
</div>
""", unsafe_allow_html=True)
...
dot_class, status_text = "idle", "Статус не определен - нажмите кнопку Пинг"
if st.session_state.health_ok is True:
    dot_class, status_text = "ok", f"C_2 онлайн · {html.escape(C2_URL.split('//')[-1])}"
elif st.session_state.health_ok is False:
    dot_class, status_text = "err", f"C_2 недоступен · {html.escape(C2_URL.split('//')[-1])}"
        ...
        with ce:
            if st.session_state.body_mode == "structured":
                st.session_state.payload_key = st.text_input(
                    ...
                    help="Под каким ключом обернуть введенный текст в JSON-тело"
                )
            else:
                st.caption(
...
with st.form(key="request_form", clear_on_submit=False):
    placeholder_text = {
        "structured": 'Введите запрос...\nБудет обернут: {"' + st.session_state.payload_key + '": "..."}',
...
if submitted:
    ...
    # Валидация endpoint
    valid, err_msg = validate_endpoint(endpoint)
    if not valid:
        log(f"Невалидный endpoint: {err_msg}", "err")
        st.error(f"Ошибка endpoint: {err_msg}")
    else:
        # Валидация длины ввода
        if len(input_text) > MAX_INPUT_LENGTH:
            log(f"Превышена длина ввода: {len(input_text)} > {MAX_INPUT_LENGTH}", "err")
            st.error(f"Длина ввода превышает {MAX_INPUT_LENGTH} символов ({len(input_text)})")
        else:
            # Rate limiting
            can_send, wait_ms = check_rate_limit()
            if not can_send:
                log(f"Rate limit: подождите {wait_ms} мс", "warn")
                st.warning(f"Слишком частые запросы. Подождите {wait_ms} мс.")
            else:
                # Валидация body
                abort = False
                ...
                if not abort:
                    ...
                    st.session_state.last_content_type = result.get("content_type", "")
...
if st.session_state.last_response is not None or st.session_state.last_error:
    ...
    if status is not None:
        ...
        metrics += f'<span class="metric-chip {status_cls}">HTTP <b>{html.escape(str(status))}</b></span>'
    if elapsed is not None:
        metrics += f'<span class="metric-chip">Время <b>{html.escape(f"{elapsed:.0f} мс")}</b></span>'
    if st.session_state.last_response is not None:
        try:
            size = len(json.dumps(st.session_state.last_response,
                                  ensure_ascii=False, default=str).encode("utf-8"))
            metrics += f'<span class="metric-chip">Размер <b>{html.escape(format_bytes(size))}</b></span>'
        except (TypeError, ValueError):
            pass
    if attempts and attempts > 1:
        metrics += f'<span class="metric-chip">Попыток <b>{html.escape(str(attempts))}</b></span>'
    if content_type:
        metrics += f'<span class="metric-chip">Content-Type <b>{html.escape(content_type[:30])}</b></span>'
    metrics += '</div>'
...
if st.session_state.history:
    with st.expander(f"◷ История запросов ({len(st.session_state.history)})",
        ...
        for item in filtered:
            ...
            safe_endpoint = html.escape(item['endpoint'])
            safe_method = html.escape(item['method'])
            safe_ts = html.escape(item['ts'])
            safe_elapsed = html.escape(f"{item['elapsed_ms']:.0f}")
            safe_status = html.escape(str(item['status']) if item['status'] is not None else "—")
            safe_attempts = html.escape(attempts_str)
            ...
            <div style="padding: 8px 12px; margin: 4px 0; border-radius: 8px;
                ...
                <span style="color: rgba(0,0,0,0.5);"> {safe_ts} </span>
                <span style="color: #007aff;">{safe_method} {safe_endpoint}</span>
                <span style="color: rgba(0,0,0,0.4);"> · {safe_elapsed} мс · HTTP {safe_status}{safe_attempts}</span>
                <div style="color: rgba(0,0,0,0.6); margin-top: 3px;">↳ {safe_preview}</div>
            </div>
            """, unsafe_allow_html=True)
...
<div class="glass-footer">
    R9 SYSTEM · НОД C_2 · {html.escape(C2_URL.split('//')[-1])} · {'TOKEN ✓' if HF_TOKEN else 'NO TOKEN'} · v3.3
</div>
